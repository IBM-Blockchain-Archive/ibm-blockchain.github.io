...
NUM_PENDING=$(kubectl get deployments | grep blockchain | wc -l | grep -o -E '[0-9]+')
...
NUM_PENDING=$(kubectl get svc | grep blockchain | wc -l | grep -o -E '[0-9]+')
while [ "${NUM_PENDING}" != "0" ]; do
...
while [ "$(kubectl get pods | grep utils | wc -l | grep -o -E '[0-9]+')" != "0" ]; do
