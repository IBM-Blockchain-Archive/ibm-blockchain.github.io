...
if [ "$(kubectl get pods -a | grep createchannel | wc -l | awk '{print $1}' | grep -o -E '[0-9]+')" != "0" ]; then
    ...
    while [ "$(kubectl get pods -a | grep createchannel | wc -l | awk '{print $1}' | grep -o -E '[0-9]+')" != "0" ]; do
...
if [ "$(kubectl get pods -a | grep joinchannel | wc -l | awk '{print $1}' | grep -o -E '[0-9]+')" != "0" ]; then
    ...
    # Wait for the pod to be deleted
    while [ "$(kubectl get pods -a | grep joinchannel | wc -l | awk '{print $1}' | grep -o -E '[0-9]+')" != "0" ]; do
