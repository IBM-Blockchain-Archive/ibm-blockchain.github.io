...
while [ "$(kubectl get svc | grep composer-identity-import | wc -l | awk '{print $1}' | grep -o -E '[0-9]+')" != "0" ]; do
...
while [ "$(kubectl get deployments | grep composer-playground | wc -l | awk '{print $1}' | grep -o -E '[0-9]+')" != "0" ]; do
...
while [ "$(kubectl get svc | grep composer-playground | wc -l | awk '{print $1}' | grep -o -E '[0-9]+')" != "0" ]; do
