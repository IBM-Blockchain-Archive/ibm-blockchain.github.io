...
while [ "$(kubectl get deployments | grep composer-rest-server | wc -l | awk '{print $1}' | grep -o -E '[0-9]+')" != "0" ]; do
...
while [ "$(kubectl get svc | grep composer-rest-server | wc -l | awk '{print $1}' | grep -o -E '[0-9]+')" != "0" ]; do
	echo "Waiting for composer rest server to be deleted"
	sleep 1;
done
